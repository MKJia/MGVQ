...
class Evaluator:
    ...
    def compute_activations(self, batches: Iterable[np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for batch in tqdm(batches):
            batch = batch.astype(np.float32) # 64,256,256,3 * 157
            pred, spatial_pred = self.sess.run(
                [self.pool_features, self.spatial_features], {self.image_input: batch}
            )
            preds.append(pred.reshape([pred.shape[0], -1])) # 64 1 1 2048
    ...
    def compute_statistics(self, activations: np.ndarray) -> FIDStatistics:
        mu = np.mean(activations, axis=0) # 50000, 2048 -> 2048
        sigma = np.cov(activations, rowvar=False) # 2048, 2048
